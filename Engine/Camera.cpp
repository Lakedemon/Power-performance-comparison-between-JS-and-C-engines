...
Vector4 Camera::clippingPlane(Transform plane, Matrix4 viewMat, unsigned int side) {
    const Vector3 pos = viewMat * plane.position;
    const Vector3 normal = ((viewMat * plane.forward()) * side).normalized();

    const float dis = -Vector3::dot(pos, normal);
    return {normal.x, normal.y, normal.z, dis};
}

Matrix4 Camera::relativeMirror(Matrix4 from, Matrix4 to) {
